const updateCartSize = async (n) => {
    const itemsNumTop = document.querySelector("div.cart-item div.items-number");
    if (itemsNumTop)
        itemsNumTop.textContent = `${n} items are in your cart`;
    const summaryWrapper = document.querySelector("div.summary-wrapper");
    if (summaryWrapper) {
        const itemNumEl = summaryWrapper.querySelector("div.item-number");
        if (itemNumEl)
            itemNumEl.textContent = n + "";
    }
};
window.addEventListener("load", () => {
    ...
    const productsContainer = document.querySelector("div.cart-product-list");
    if (productsContainer) {
        ...
        const removeAllBtn = document.querySelector("div.cart-item a.remove-all-link");
        if (removeAllBtn) {
