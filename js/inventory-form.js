    ...
    return new Promise((res, rej) => {
        fetch(`${baseUrl}/api/trailer`, { credentials: "include" })
            ...
            if (data.status) {
                ...
                rej(new Error(data.msg));
        });
    });
};
const populateModal = (inventory) => {
    const modalEl = document.querySelector("div.user-management-modal");
    if (modalEl) {
...
window.addEventListener("load", async () => {
    const inventoryFormEl = document.querySelector("div.inventory-content form");
    const reviewFormEl = document.querySelector("div.user-management-modal form");
    if (inventoryFormEl && reviewFormEl) {
        const inventoryContainer = inventoryFormEl.querySelector("div.inventory-parent");
        try {
            inventoryFormEl
        ...
        console.error(new Error("Inventory form element missing!"));
});
