...
window.addEventListener("load", async () => {
  try {
    ...
    const resp: Awaited<SubmissionsAPIRespT> = await req.json();

    if (!resp.status) throw new Error(resp.msg);

    const {
      data: { trailers, records, categories },
    } = resp;

    const dashboardContainer =
      document.querySelector<HTMLDivElement>(".dashboard-panel");

    const existingEls = dashboardContainer.querySelectorAll(
      ".category-row, .category-column"
    );
    existingEls.forEach((el) => el.remove());

    const panelHeading = document.querySelector<HTMLHeadingElement>(
      ".panel-title-flex h1"
    );
    ...
    {
      const exportBtn =
        document.querySelector<HTMLAnchorElement>(".export-btn");
      if (exportBtn) {
        const href = exportBtn.getAttribute("href");
        if (href === "#") {
          const fileReq = await fetch(
            `${baseUrl}/api/inventory/submissions-csv`,
            { credentials: "include" }
          );
