...
type FetchUsersAPIRespT = {
  status: boolean;
  data: UserT[];
  msg: string;
};
type AddUserFormDataT = {
  email: string;
  role: number;
};
type AddUserAPIRespT = {
  status: boolean;
  msg: string;
};
type DeleteUserAPIRespT = {
  status: boolean;
  msg: string;
};
...
const deleteUser = async (email: string, itemEl: HTMLDivElement) => {
  try {
    ...
    const res: Awaited<DeleteUserAPIRespT> = await req.json();
...
const renderUsers = (users: UserT[]) => {
  try {
    ...
    } else throw new Error("Users container element missing!");
  } catch (error) {
    console.error(error);
  }
};
...
window.addEventListener("load", async () => {
  try {
    ...
    const resp: Awaited<FetchUsersAPIRespT> = await req.json();
    if (resp.status) {
      ...
      if (formEl) {
        formEl.addEventListener("submit", async (ev) => {
          ...
          try {
            const formData: AddUserFormDataT = {
              email: "",
              role: 0,
            };
            ...
            const roleEl =
              formEl.querySelector<HTMLSelectElement>(`select[name="field"]`);
            if (roleEl) formData.role = +roleEl.value;
            ...
            const resp: Awaited<AddUserAPIRespT> = await req.json();
            if (!resp.status) throw new Error(resp.msg);
            alert("User added successfully!");

            const closeBtn = document.querySelector<HTMLDivElement>(
              "div.user-management-modal .close-btn"
            );
